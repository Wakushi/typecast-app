...
export function VideoPlayerClient({
  ...
  const [isHlsVideoElementLoaded, setIsHlsVideoElementLoaded] =
  ...
  const loadScript = (src: string) => {
    return new Promise((resolve, reject) => {
      ...
      script.onerror = () =>
        reject(
          new Error(`[react-farcaster-embed] Script load error for ${src}`)
        )
  ...
  React.useEffect(() => {
    ...
    const hlsVideoElement =
      "https://cdn.jsdelivr.net/npm/hls-video-element@1.0/+esm"
